...
const verifyDocumentRelation = async (upn, id, groups, documentId, recno) => {
  ...
  const documentMatch = student.documents.find(doc => doc.documentNumber === documentId)
...
const getFile = async (source, documentId, recno, upn, id, groups) => {
  ...
  if (!verifiedDocumentRelation) {
    logger('error', ['handler', 'getFile', 'verifyDocumentRelation', 'ingen tilgang', 'Dokument- eller filnummeret tilhører ikke denne eleven'])
