...
module.exports = async fnr => {
  ...
  await Promise.all(config.P360.map(async (source) => {
    logger('info', ['retrieve-documents', source.name, source.type])

    try {
      const client = p360(source)
      const { documents } = await client.getDocuments(fnr)
      if (!documents) {
        logger('info', ['retrieve-documents', source.name, source.type, 'no documents found'])
        return
      }

      logger('info', ['retrieve-documents', source.name, source.type, 'documents', documents.length])
      ...
      logger('error', ['retrieve-documents', source.name, source.type, error])
    }
  }))
